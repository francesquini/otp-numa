...
#ifdef ERTS_SMP
ERTS_GLB_INLINE void schedule_wake_up_schedulers(struct scheduling_data* sd) {
	int wo_reds = sd->rq->wakeup_other_reds;
	if (wo_reds) {
		if (sd->rq->len < 2) {
			sd->rq->wakeup_other -= ERTS_WAKEUP_OTHER_DEC*wo_reds;
			if (sd->rq->wakeup_other < 0)
				sd->rq->wakeup_other = 0;
		}
		else if (sd->rq->wakeup_other < wakeup_other_limit)
			sd->rq->wakeup_other += sd->rq->len*wo_reds + ERTS_WAKEUP_OTHER_FIXED_INC;
		else {
			if (erts_smp_atomic32_read_acqb(&no_empty_run_queues) != 0) {
				wake_scheduler_on_empty_runq(sd->rq);
			}
			sd->rq->wakeup_other = 0;
		}
	}
...
#ifdef ERTS_SMP
		schedule_wake_up_schedulers(sd);
#endif
